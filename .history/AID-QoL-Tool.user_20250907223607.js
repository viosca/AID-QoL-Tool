    ...
    const observerCallback = (mutationsList, observer) => {
        ...
        for (const mutation of mutationsList) {
            ...
            if (mutation.type === 'attributes' && mutation.attributeName === 'placeholder') {
                ...
                // Check if the mode was just switched to "Do".
                if (newPlaceholder === 'What do you do?') {
                    // Only modify if the textarea is empty.
                    if (textarea.value.trim() === '') {
                        // Use the script's existing setNativeValue function to ensure React sees the change.
                        setNativeValue(textarea, '""');

                        // We need a brief delay for React to process the value change
                        // before we can focus and set the cursor position.
                        setTimeout(() => {
                            textarea.focus();
